...
namespace Lab01.Tests
{
    public class UnitTests
    {
            ...
            var testLines = new List<string>
        {
            ...
            Assert.Equal(2, result.Count); // Check that got results for both lines
            Assert.NotEmpty(result[0]); // Check that result is not empty
            Assert.NotEmpty(result[1]); // Check that result is not empty
